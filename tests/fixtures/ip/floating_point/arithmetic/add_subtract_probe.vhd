...
entity tb_floating_add_probe is
end entity;

architecture sim of tb_floating_add_probe is
  ...
  constant inputs : lanes_t := (
    (x"00", x"80", x"38", x"38", x"39", x"3F", x"77", x"09", x"78", x"78", x"79", x"01"),
    (x"80", x"80", x"38", x"18", x"18", x"18", x"77", x"08", x"F8", x"78", x"38", x"B8"),
    (x"00", x"40", x"81", x"C0", x"00", x"40", x"80", x"C1", x"00", x"41", x"80", x"C0"));
  constant expected : words_t := (x"00", x"80", x"00", x"38", x"3A", x"40",
                                  x"78", x"00", x"7C", x"7C", x"7C", x"B8");
  type flags_t is array(0 to 11) of std_logic_vector(2 downto 0);
  constant flags : flags_t := (6 => "010", 7 => "001", 8 | 9 => "100", others => "000");
  ...
  signal sink_user : std_logic_vector(12 downto 0);
begin
      ...
      for i in expected'range loop
        ...
        loop
          ...
          assert ready(lane) = '0' or ready(lane) = '1'
            report "FLOATING_ADD_PROBE_STATUS: FAIL unknown input ready" severity failure;
  ...
  process
    ...
    variable held : std_logic_vector(21 downto 0);
  begin
    wait until resetn = '1';
    loop
      wait until rising_edge(clk);
      assert sink_valid = '0' or sink_valid = '1'
        report "FLOATING_ADD_PROBE_STATUS: FAIL unknown valid" severity failure;
      if stalled then
        assert sink_valid = '1' and (sink_data & sink_user & sink_last) = held
          report "FLOATING_ADD_PROBE_STATUS: FAIL changed under backpressure" severity failure;
      end if;
      ...
      if sink_valid = '1' and sink_ready = '1' then
        assert count < expected'length
          report "FLOATING_ADD_PROBE_STATUS: FAIL extra output" severity failure;
        tag := count mod 8 + ((3 * count) mod 32) * 8 + (count mod 4) * 256;
        if count mod 8 = 0 then ending := '0'; else ending := '1'; end if;
        assert sink_data = expected(count) and sink_user = (std_logic_vector(to_unsigned(tag, 10)) & flags(count))
          and sink_last = ending
          report "FLOATING_ADD_PROBE_STATUS: FAIL sample " & integer'image(count) severity failure;
        wait for 0 ns;
        assert count < accepted(0) and count < accepted(1) and count < accepted(2)
          report "FLOATING_ADD_PROBE_STATUS: FAIL result before accepted operands" severity failure;
        report "FLOATING_ADD_SAMPLE " & integer'image(count) & " PASS" severity note;
        count := count + 1;
      end if;
    ...
    report "FLOATING_ADD_PROBE_STATUS: PASS" severity note;
    finish;
    wait;
  end process;
  process
  begin
    wait for 100 us;
    assert false report "FLOATING_ADD_PROBE_STATUS: FAIL timeout" severity failure;
    wait;
  end process;
end architecture;
