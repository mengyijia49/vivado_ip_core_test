...
begin
  ...
  begin
    ...
    value := x"00";
    case key is
      when x"0080" & "010100" | x"3840" & "001100" | x"B8C0" & "100100" |
           x"7938" & "000100" | x"7938" & "101100" | x"7878" & "010100" |
           x"3839" & "011100" | x"0100" & "010100" => value := x"01";
      when x"8000" & "101100" | x"F878" & "110100" | x"C0B8" & "100100" |
           x"4038" & "011100" => value := x"00";
    ...
    if fault_mode = 11 then value(0) := qo(rd mod 4)(3); end if;
    if fault_mode = 12 then value(7) := '1'; end if;
    m_axis_result_tdata <= value;
    m_axis_result_tuser <= users;
