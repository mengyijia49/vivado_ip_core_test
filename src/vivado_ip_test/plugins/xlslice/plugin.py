...
from vivado_ip_test.plugins.xlslice.reference import SliceModel


class XlSlicePlugin(CycleIpPlugin):
    ip_type = ip_name = "xlslice"
    version = "1.0"

    def describe(self, p):
        validate_parameters(p, {"input_width": range(2, 4097), "high_bit": range(256), "low_bit": range(256)})
        width, high, low = p["input_width"], p["high_bit"], p["low_bit"]
        if not low <= high < width:
            raise PluginError("Slice requires 0 <= low_bit <= high_bit < input_width")
        ...
        return CycleSpec(ports, (Port("Dout", high-low+1),), settings, settings,
            lambda: SliceModel(low, high-low+1), clock=None, prefix=prefix,
