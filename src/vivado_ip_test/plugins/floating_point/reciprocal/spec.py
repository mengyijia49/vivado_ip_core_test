...
def describe(p):
    validate_parameters(p, {'operation': {'Reciprocal'},
    ...
    if p['has_overflow'] or p['has_invalid_op']:
        raise PluginError('Reciprocal does not provide OVERFLOW or INVALID_OP flags')
    ...
    if source_format is None or result_format is None:
        raise PluginError('Reciprocal supports only native Half, Single, and Double formats')
    if source_format != result_format:
        raise PluginError('Reciprocal mixed formats are not supported by this behavioral test')
    ...
    selected = [name for name in ('underflow', 'divide_by_zero') if p['has_' + name]]
    ...
    settings = {'Operation_Type': 'Reciprocal', 'A_Precision_Type': source_format,
        ...
        'C_Has_UNDERFLOW': p['has_underflow'], 'C_Has_OVERFLOW': False,
        'C_Has_INVALID_OP': False, 'C_Has_DIVIDE_BY_ZERO': p['has_divide_by_zero'],
    ...
    model = {**{f'C_HAS_{name}': int(name == 'RECIP') for name in ALL_OPERATIONS},
        ...
        'C_HAS_UNDERFLOW': int(p['has_underflow']), 'C_HAS_OVERFLOW': 0,
        'C_HAS_INVALID_OP': 0, 'C_HAS_DIVIDE_BY_ZERO': int(p['has_divide_by_zero']),
    ...
    return StreamSpec(tuple(source), settings, model, output_payload=tuple(sink),
        input_prefix='s_axis_a', output_prefix='m_axis_result', capacity=32, drain_cycles=128)
