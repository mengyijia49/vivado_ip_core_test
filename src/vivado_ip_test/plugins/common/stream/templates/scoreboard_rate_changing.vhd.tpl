  scoreboard : process
    ...
    variable actual, expected, held : std_logic_vector($output_width - 1 downto 0);
...
$captures
      ...
      if sampled_valid = '1' and sampled_ready = '1' then
        ...
        flush(actual_file);
        assert not endfile(expected_file) and not endfile(causal_file)
          report "AXIS_SELF_CHECK_STATUS: FAIL extra output" severity failure;
        readline(expected_file, row);
        read(row, expected);
        readline(causal_file, row);
        read(row, required_count);
    ...
    assert endfile(expected_file) and endfile(causal_file) and sent = count
