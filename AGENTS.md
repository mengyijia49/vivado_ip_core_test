...
当前接入 64 类 IP，各有 Python 参考模型。默认仍为原有 6 组配置、19 个阶段；
`--all` 选择 478 组常用配置，`--ip-type` 可单独运行某类 IP。
...
AXI Protocol Checker 从设备侧测试合法事务和十类请求协议违规，逐项核对锁存状态位。
AXI BRAM Controller 使用内部 BRAM，检查 AXI4-Lite 地址、字节选通、保护属性和复位保持。
LMB BRAM Interface Controller 检查地址译码、访问保护、应答和外部 BRAM 端口转发。
AXI Timebase Watchdog 检查计数周期、启停、首次到期、复位锁存、冻结和总线响应。
FIT Timer 检查周期中断、单周期脉宽、允许误差、复位极性和运行中复位恢复。
Mutex 使用多路 AXI-Lite 自检共享锁、所有者、硬件保护、USER 寄存器和仲裁。
Mailbox 使用双 AXI-Lite 自检双向 FIFO、满空错误、清除、阈值中断和响应回压。
Util FF 已接入四种触发器和两种锁存器，检查初值、使能、控制极性、数据和门控反相。
AXI-to-APB Bridge 已接入 APB4，检查地址窗口、字节写、保护属性、错误映射、等待时序和多从设备路由。
AXI4-Stream FIFO 通过 AXI-Lite 写入和读回数据包，检查长度、数据、TKEEP、TDEST、回压和中断。
AXI Sideband Utility 检查五个 AXI4 通道、SMID 旁路、插入、移除和双向回压。
AXI Register Slice 检查五个 AXI4 通道的五种寄存模式、停顿保持、吞吐和复位。
AXI Protocol Converter 检查 AXI4 burst 到 AXI4-Lite 单拍访问的拆分、地址和响应合并。
...
Mailbox 同时接入双 AXI-Lite 和同步双向 AXI4-Stream；流模式检查跨侧路由、
TLAST、满队列限流、输出回压、双向并发和复位清空。
Mailbox 流模式的 TLAST 丢失已在分布式 RAM 和块 RAM 中复现，按待确认问题保留。
...
TMR Injector 检查控制写、目标地址、一次性指令替换、保护属性和三路 LMB 透明转发。
CORDIC 平方根和正余弦接入独立数学参考及 Blocking 握手检查；正余弦只生成合法角度，
两个有符号结果字段各允许 2 LSB。Nearest_Even 差异不能直接计作 IP bug。
Floating-Point 接入绝对值、三种格式转换、平方根、比较、加减、乘法及除法。
加减与乘法使用精确整数参考，复用比较器的独立输入驱动，检查舍入、异常位和侧带。
除法通过整数商余数判断舍入，覆盖除零、无效运算、全部合法运算间隔和独立输入握手。
融合乘加/减使用精确乘积与和，只在最终结果舍入，并独立驱动 A、B、C 和操作通道。
倒数和倒数平方根使用精确整数参考；单、双精度按 PG060 允许的 1 ULP 范围评分。
指数和自然对数使用高精度十进制参考；普通有限结果按 PG060 的 1 ULP 范围评分。
双精度低延迟乘法的数值异常有固定输入独立 VHDL 复现，速度优化对照正常，尚待确认。
FIR 全精度负系数边界有独立 VHDL 数值差异，两种架构均复现，正系数对照正常。
FIR 已接入单速率、固定整数系数和全精度输出；独立复现命令继续用于固定问题样本。
DDS 已接入固定步进和固定相偏的相位发生器，检查回绕、回压及寄存复位时序。
Processor System Reset 检查复位脉冲过滤、输入极性、锁定丢失、释放顺序和输出副本。
Clocking Wizard 检查 MMCM/PLL 输出周期、占空比、锁定、复位解锁和重新锁定。
CIC Compiler 检查固定倍率抽取和插值、全精度数值、输出数量、回压及输入因果关系。
AXI UART Lite 通过串行回接检查字符宽度、校验位、收发 FIFO、状态和总线响应。
卷积编码器检查约束寄存器、多项式奇偶校验、1/2 至 1/7 输出率和流接口回压。
FFT 检查无缩放定点正逆变换、输出顺序、帧边界、配置握手和回压。
...
1. Python 总控，Vivado 只能通过 batch Tcl 调用，不依赖 GUI。
   后续运行只使用 /data/Xilinx/2026.1/Vivado/settings64.sh；入口拒绝其他版本。
...
5. 所有运行产物放入 runs/，日志放入 runs/logs/，报告放入 reports/。
   ...
   新工程位于 runs/batches/<vivado_version>/<run_id>/<ip_type>/<case_id>/。
   新日志位于 runs/logs/batches/<vivado_version>/<run_id>/<ip_type>/<case_id>/。
   新报告位于 reports/history/<vivado_version>/<run_id>/。
   reports/latest 和 reports/by_version/<vivado_version>/latest 只作符号链接。
